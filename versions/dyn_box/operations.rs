...
impl<Lhs: Expr, Rhs: Expr> Expr for AddExpr<Lhs, Rhs> {
    fn value(&self, bindings: &Bindings) -> Box<dyn Expr> {
        let left = (self.left.value(bindings) as Box<dyn Any>).downcast::<IntExpr>().unwrap();
        let right = (self.right.value(bindings) as Box<dyn Any>).downcast::<IntExpr>().unwrap();
...
impl<Lhs: Expr, Rhs: Expr> Expr for AndExpr<Lhs, Rhs> {
    fn value(&self, bindings: &Bindings) -> Box<dyn Expr> {
        let left = (self.left.value(bindings) as Box<dyn Any>).downcast::<BoolExpr>().unwrap();
        let right = (self.right.value(bindings) as Box<dyn Any>).downcast::<BoolExpr>().unwrap();
...
impl<E: Expr> Expr for NotExpr<E> {
    fn value(&self, bindings: &Bindings) -> Box<dyn Expr> {
        let expr = (self.expr.value(bindings) as Box<dyn Any>).downcast::<BoolExpr>().unwrap();
